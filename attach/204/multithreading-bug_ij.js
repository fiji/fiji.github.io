...
//	var stack = img.getStack();
	...
	var pixelWidth = dapiImg.getCalibration().pixelWidth;
	var pixelHeight = dapiImg.getCalibration().pixelHeight;
	var imageWidth = dapiImg.getWidth();  
	var imageHeight = dapiImg.getHeight();  
	...
	if (firstImgProcessed == false)
	{
		IJ.run(dapiImg, "Gaussian Blur...", "sigma="+nucleiSigma);
		IJ.setAutoThreshold(dapiImg, nucleiThresholder);

		lowerDThreshold = dapiImg.getProcessor().getMinThreshold();
		// very bright nuclei appear to be dead, remove them
		if (removeBrightNuclei) {
				IJ.setAutoThreshold(dapiImg, brightNucleiThresholder);
				upperDThreshold = dapiImg.getProcessor().getMinThreshold();
		} else {
			upperDThreshold = dapiImg.getProcessor().getMaxThreshold();
		}
	}

	IJ.setThreshold(dapiImg, lowerDThreshold, upperDThreshold);
	...
	if (firstImgProcessed == false) 	{
		IJ.setThreshold(phdnImg, 1, 65535); // ignore black edge frames here
		IJ.setAutoThreshold(phdnImg, phdnThresholder);
		lowerPhdnThreshold = phdnImg.getProcessor().getMinThreshold();
		upperPhdnThreshold = phdnImg.getProcessor().getMaxThreshold();

	} else	{
		IJ.setThreshold(phdnImg, lowerPhdnThreshold, upperPhdnThreshold);
	}
    ...
    //  create a roi for cmcs from troponin t staining 
	if (firstImgProcessed == false)	{
		IJ.setAutoThreshold(cytoplasmImg, trtThresholder);		
		lowerCytoThreshold = cytoplasmImg.getProcessor().getMinThreshold();
		upperCytoThreshold = cytoplasmImg.getProcessor().getMaxThreshold();
		firstImgProcessed = true;
	}	else	{
		IJ.setThreshold(cytoplasmImg, lowerCytoThreshold, upperCytoThreshold);
	}
	...
	var trtMeasurements = Measurements.MEAN + Measurements.AREA + Measurements.CIRCULARITY;
	var rt_trt = new ResultsTable();
	var trtAnalyzer = new Analyzer(trtImg, trtMeasurements, rt_trt); 
	...
	for (var i = 0; i < rois.length; i++) { 
		rois[i].setStrokeColor(red); 
		trtImg.setRoi(rois[i]);
		dapiImg.setRoi(rois[i]);
		trtAnalyzer.measure();
		trtImg.deleteRoi();
		if(testing) {
			rt_trt.show("TRT measurements");
		}
		var trtArea = rt_trt.getValueAsDouble(rt_trt.getColumnIndex("Area"), i);
		var trtMeanIntensity = rt_trt.getValueAsDouble(rt_trt.getColumnIndex("Mean"), i);
		var trtRoundness = rt_trt.getValueAsDouble(rt_trt.getColumnIndex("Round"), i);
		var CMsmdIntensity = -1;
		if(channels.length == 4) {
			var analyzer_SMD = new Analyzer(smdImg, Measurements.MEAN, rt_cmcsSMD);
			smdImg.setRoi(rois[i]);
			analyzer_SMD.measure();
			CMsmdIntensity = rt_cmcsSMD.getValueAsDouble(rt_cmcsSMD.getColumnIndex("Mean"), i);
		} 
...
var lightblue = new Color(0, 0.33, 0.72);
var white = new Color(1, 1, 1);
var lightgray = new Color(0.8, 0.8, 0.8);
